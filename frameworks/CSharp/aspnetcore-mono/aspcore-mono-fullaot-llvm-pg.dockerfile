...
# AOT the framework.
RUN i=/usr/lib/mono/4.5/mscorlib.dll && echo "=====" && echo "Starting AOT: $i" && echo "=====" && mono --aot=fullaot,llvm,stats $i && echo ""; done
RUN for i in /usr/lib/mono/gac/*/*/*.dll; do echo "=====" && echo "Starting AOT: $i" && echo "=====" && mono --aot=fullaot,llvm,stats $i && echo ""; done
...
# AOT the test.
RUN for i in *.dll; do echo "=====" && echo "Starting AOT: $i" && echo "=====" && mono --aot=fullaot,llvm,stats $i && echo ""; done
RUN for i in *.exe; do echo "=====" && echo "Starting AOT: $i" && echo "=====" && mono --aot=fullaot,llvm,stats $i && echo ""; done
