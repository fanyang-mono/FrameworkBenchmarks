...
RUN export MONO_PATH=/mono/mcs/class/lib/net_4_x-linux && \
    export export PATH=/mono/llvm/usr/bin:$PATH && \
    for i in *.dll; do echo "=====" && echo "Starting AOT: $i" && echo "=====" && /mono/mono/mini/mono --aot=llvm $i && echo ""; done && \
    for i in *.exe; do echo "=====" && echo "Starting AOT: $i" && echo "=====" && /mono/mono/mini/mono --aot=llvm $i && echo ""; done
